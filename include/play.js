...
const { canModifyQueue } = require("../util/CnrXbotUtil.js");
...
module.exports = {
  async play(song, message) {
    ...
    if (!song) {
      ...
      return queue.textChannel.send(" Müzik kuyruğu sona erdi.").catch(console.error);
    }
    ...
    try {
      var playingMessage = await queue.textChannel.send(` Müzik Başlatıldı: **${song.title}** ${song.url}`);
      ...
      switch (reaction.emoji.name) {
        ...
        case "":
          ...
          if (queue.playing) {
            ...
            queue.textChannel.send(`${user} ▶ müziği yeniden başlattı!`).catch(console.error);
          }
          break;
        ...
        case "":
          ...
          queue.textChannel.send(`Döngü: ${queue.loop ? "**on**" : "**off**"}`).catch(console.error);
          break;
